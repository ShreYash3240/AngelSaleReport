...
function renderReportTable(bills) {
    ...
    bills.forEach(bill => {
        ...
        // S3 Receipt Vault view action (triggers the popup modal)
        const receiptAction = bill.receiptS3Key 
            ? `<button type="button" class="clear-btn" style="padding: 4px 10px; background: #e0f2fe; color: #0284c7; border: 1px solid #bae6fd; border-radius: 6px; font-weight: 600; font-size: 0.75rem; cursor: pointer;" onclick="viewReceiptImage('${escapeHTML(bill.receiptS3Key)}')" title="View Digital Receipt Slip">📷 View</button>` 
